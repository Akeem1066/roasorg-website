...
export default async function handler(req, res) {
  ...
  try {
    ...
    res.status(200).json({ 
      ...
      url: session.url,
      success: true,
      message: 'Checkout session created successfully (mobile redirect)'
    });
  } catch (error) {
    console.error('Error creating checkout session (mobile redirect):', error);
